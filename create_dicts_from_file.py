...
'''elif line.startswith("<OUTLINK>"):
            print "inside outlink"
            out_link_bool = True
            out_link_string += line[9:]
        elif line.startswith("<INLINK>"):
            print "inside inlink"
            in_link_bool = True
            in_link_string += line[8:]
        elif "</INLINK>" in line:
            print "exiting inlink"
            in_link_bool = False
            in_link_string += line[:-10]
            in_link_string = in_link_string.replace(", u",", ")
            in_link_string = in_link_string.replace("[u","")
            in_link_string = in_link_string.replace("]","")
            in_link_string = in_link_string.replace("'","")
            ...
            in_link_string += line
        elif "</OUTLINK>" in line:
            out_link_bool = False
            out_link_string += line[:-11]
            out_link_string = out_link_string.replace(", u",", ")
            out_link_string = out_link_string.replace("[u","")
            out_link_string = out_link_string.replace("]","")
            out_link_string = out_link_string.replace("'","")
            ...
            out_link_list = out_link_string.split(",")
            outlinks = [unicode(x) for x in out_link_list]
            print "exiting outlink"
            break
        elif out_link_bool:
            out_link_string += line'''